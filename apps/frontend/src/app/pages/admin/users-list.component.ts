...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
      ...
      <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p class="text-muted-foreground mt-1">
            Gerencie os usuários cadastrados, aprove cadastros e redefina senhas.
          </p>
        </div>
      </div>
      ...
      @if (successMsg()) {
        <div
          class="mb-6 bg-success/10 border border-success/20 text-success px-4 py-3.5 rounded-xl flex items-center justify-between shadow-sm"
        >
                ...
                <tr class="hover:bg-muted/50 transition-colors">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      <div
                        class="w-10 h-10 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-bold"
                      >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-2">
                      @if (user.role !== 'admin') {
                        <button
                          ui-button
                          [variant]="user.ativo ? 'destructive' : 'success'"
                          size="sm"
              ...
              <div class="p-5 space-y-4 hover:bg-muted/50 transition-colors">
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-border">
                  ...
                  <button
                    ...
                    variant="secondary"
                    size="sm"
                    [ngClass]="{ 'w-full': user.role === 'admin', 'flex-1': user.role !== 'admin' }"
                    (click)="resetPassword(user.id)"
                    [disabled]="actionLoading() === user.id"
                  >
        ...
        @if (isLoading() && filteredUsers().length === 0) {
          <div class="py-20 flex flex-col items-center justify-center gap-3">
            ...
            <span class="text-muted-foreground text-sm font-medium"
              >Buscando lista de usuários...</span
            >
          </div>
        }
        ...
        @if (!isLoading() && filteredUsers().length === 0) {
          <div class="py-20 flex flex-col items-center justify-center text-center p-6">
            ...
            <p class="text-muted-foreground text-sm max-w-sm">
              Tente redefinir a busca ou verifique se há usuários cadastrados.
            </p>
          </div>
        }
      </ui-card>
...
export class UsersListComponent implements OnInit {
  ...
  toggleActivation(user: User) {
    ...
    // O endpoint PATCH /users/:id/activate alterna o estado (ativa/desativa).
    const willActivate = !user.ativo;
    this.authService.activateUser(user.id).subscribe({
      next: () => {
        this.successMsg.set(
          willActivate
            ? 'Usuário ativado com sucesso e agora tem acesso ao portal!'
            : 'Usuário desativado. O acesso ao portal foi revogado.',
        );
